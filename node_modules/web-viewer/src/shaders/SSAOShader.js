...
THREE.SSAOShader = {
	...
	fragmentShader: [
		...
		"vec4 blur( sampler2D texture, vec2 uv, vec2 s ) {",
		"	vec4 sum = vec4( 0.0 );",
		"	sum += texture2D( texture, vec2( uv.x - s.x, uv.y + s.y ) ) * 0.0947416;",
		"	sum += texture2D( texture, vec2( uv.x, uv.y + s.y ) ) * 0.118318;",
		"	sum += texture2D( texture, vec2( uv.x + s.x, uv.y + s.y ) ) * 0.0947416;",

		"	sum += texture2D( texture, vec2( uv.x - s.x, uv.y ) ) * 0.118318;",
		"	sum += texture2D( texture, vec2( uv.x, uv.y ) ) * 0.147761;",
		"	sum += texture2D( texture, vec2( uv.x + s.x, uv.y ) ) * 0.118318;",
		...
		"float readDepth( const in vec2 coord ) {",
			...
			//"vec4 depthColor = blur( tDepth, coord, vec2(1.0 / size.x, 1.0 / size.y) );",
			"vec4 depthColor = texture2D( tDepth, coord );",

			"#ifdef USE_LOGDEPTHBUF",

				"float logz = unpackRGBAToDepth( depthColor );",
				"float w = pow(2.0, (logz / logDepthBufFC)) - 1.0;",
				"float z = (logz / w) + 1.0;",

			"#else",

				"float z = unpackRGBAToDepth( depthColor );",
		...
		"void main() {",
			...
			"vec4 color = texture2D( tDiffuse, vUv ).rgba;",
